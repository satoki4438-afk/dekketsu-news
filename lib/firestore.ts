...
type ArticleInput = Omit<Article, "id" | "createdAt" | "publishedAt"> & { publishedAt: string };

export async function saveArticles(articles: ArticleInput[]): Promise<Article[]> {
...
export async function getArticlesByCategory(category: string, limit = 6): Promise<Article[]> {
  const snapshot = await db
    .collection("articles")
